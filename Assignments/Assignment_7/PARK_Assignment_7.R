...
# vanilla: buddhist = utah[utah$Buddhism.Mahayana > 0,]
buddhist <- utah %>% filter(Buddhism.Mahayana > 0)

# order rows by population (descending)
# vanilla: buddhist = buddhist[order(buddhist$Pop_2010, decreasing = TRUE),]
...
## Vanilla: group1 = mean(utah[utah$Pop.Group == 1,]$Religious)
...
# vanilla: group1.pop = mean(utah[utah$Pop.Group == 1,]$Pop_2010)
...
Pop.mean.group <- 
  utah %>% 
  group_by(Pop.Group) %>%
  summarize(mean(Pop_2010))



# make data frame of each group and mean religiosity
# vanilla: religiosity = data.frame(Pop.Group = c("group1","group2","group3","group4","group5","group6"),
...
religiosity <- 
  inner_join(Religous.mean.group,Pop.mean.group)


religiosity # take quick look at resulting table

# order by decreasing population
# vanilla: religiosity = religiosity[order(religiosity$Mean.Pop, decreasing = TRUE),]

religiosity <- 
  religiosity %>% 
  arrange(desc(`mean(Pop_2010)`))
